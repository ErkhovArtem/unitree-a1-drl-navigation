...
Usage:
    ...
    python scripts/inference_onnx.py   # defaults: sac_actor.onnx, configs/a1.yaml
    ...
    def build_observation(
        ...
        if self.history_length > 0 and self.action_history is not None:
            if len(self.action_history) == 0:
                ...
                action_history_flat = np.concatenate(list(self.action_history)).astype(np.float32)
            ...
            obs = np.concatenate([base_obs, action_history_flat]).astype(np.float32)
        else:
            obs = base_obs
...
def _resolve_path(value: str, kind: str, candidates: list) -> Path:
    """Resolve path trying candidates; raise FileNotFoundError if missing."""
    for p in candidates:
        r = Path(p).resolve()
        if r.exists() and r.is_file():
            return r
    raise FileNotFoundError(
        f"{kind} not found: {value}\nTried: {[str(Path(x).resolve()) for x in candidates]}"
    )
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='sac_actor.onnx',
                        help='Path to ONNX model (default: sac_actor.onnx)')
    parser.add_argument('--config_path', type=str, default='configs/a1.yaml',
                        help='Path to config YAML (default: configs/a1.yaml)')
    ...
    config_candidates = [
        ...
        PROJECT_ROOT / args.config_path,
        PROJECT_ROOT / 'configs' / Path(args.config_path).name,
    ]
    model_candidates = [
        args.model_path,
        Path.cwd() / args.model_path,
        PROJECT_ROOT / args.model_path,
        PROJECT_ROOT / Path(args.model_path).name,
    ]
    config_path = _resolve_path(args.config_path, 'Config', config_candidates)
    model_path = _resolve_path(args.model_path, 'Model', model_candidates)
    
    # Initialize inference
    inference = SACInference(str(model_path), str(config_path), args.history_length)
