...
class Lidar2DProcessor(Node):
    ...
    3. Публикует 40 секторных дистанций в метрах для политики (lidar_observations)

    Совместимость с инференсом (inference_onnx / POLICY_SPEC):
    - Выход: 40 float32, метры, [0.25, 3.0] или 3.0 при пустом секторе.
    - Секторы: [-π, π], 40 штук, минимум по сектору. Логика = process_lidar_to_sectors.
    - Инференс сам нормализует (/-max_range, *2-1 → [-1,1]). Облако должно быть в base_link (x вперёд, y влево).
    """
        ...
        try:
            ...
            if floor_points_2d is not None and len(floor_points_2d) > 0:
                ...
                if non_zero_mask.sum() == 0:
                    ...
                    # Важно: сбрасываем лидар в max_range, иначе опубликуем устаревшие значения
                    self.sector_min_distances = np.full(self.num_sectors, self.max_range, dtype=np.float32)
                else:
            ...
            # Формат совпадает с process_lidar_to_sectors / inference_onnx: 40 секторов, метры.
            # Подписчик (инференс) нормализует сам. Пустой сектор = max_range (3.0 м).
