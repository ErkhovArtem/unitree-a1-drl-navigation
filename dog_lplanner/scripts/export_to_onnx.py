...
ПРОСТОЙ ВАРИАНТ (пути относительно корня проекта):
...
С ПРОВЕРКОЙ МОДЕЛИ (требует: pip install onnxruntime):
    python scripts/export_to_onnx.py --model_path data/models/sac_actor.pth --config_path configs/a1.yaml --verify

С ПОЛНЫМИ ПУТЯМИ:
    python scripts/export_to_onnx.py --model_path /path/to/sac_actor.pth --config_path /path/to/a1.yaml --output_path sac_actor.onnx
...
def export_to_onnx(
    ...
    Args:
        ...
        config_path: Путь к конфигурационному файлу (a1.yaml)
    ...
    # Параметры из конфига (соответствие configs/a1.yaml)
    sac_config = config.get('sac', {})
    actor_config = sac_config.get('actor', {})
    cmd_scale = config.get('cmd_scale', [0.8, 0.4, 0.35])  # [vx, vy, w] из a1.yaml
    ...
    if verbose:
        ...
        if actor_obs_dim == 48:
            ...
            print(f"  ✓ Модель с историей действий: base 47 + history {hist}*3 = {actor_obs_dim}")
        print()
        ...
        print(f"5. Масштабируйте действия по cmd_scale из config (a1.yaml): "
              f"vx=action[0]*{cmd_scale[0]}, vy=action[1]*{cmd_scale[1]}, w=action[2]*{cmd_scale[2]}")
        print()
...
def main():
    ...
    parser.add_argument(
        ...
        help='Путь к конфигурационному файлу (a1.yaml)'
    )
    parser.add_argument(
        '--output_path',
        type=str,
    ...
    # Разрешаем пути относительно PROJECT_ROOT (как в train.py)
